...
(function () {
  ...
  var STEPS = [
    ...
    { key: 'weight', q: 'Вес ездока?', opts: [
      ['kid', 'До 50 кг · ребёнок/подросток'], ['l', '55–75 кг'], ['m', '80–100 кг'], ['h', 'Больше 100 кг'] ] },
    { key: 'budget', q: 'Бюджет?', opts: [
      ['b0', 'До 30 000 ₽'], ['b1', '30–50 000 ₽'], ['b2', '50–90 000 ₽'], ['b3', '90 000 ₽ и выше'], ['any', 'Не важно'] ] },
  ...
  function score(c) {
    ...
    // бюджет (мягко — чтоб всегда были варианты; b0 = бюджетный сегмент, в т.ч. AOVO)
    if (ans.budget === 'b0') sc += p <= 30000 ? 5 : (p <= 42000 ? 1 : -4);
    else if (ans.budget === 'b1') sc += (p > 25000 && p <= 50000) ? 4 : (p <= 65000 ? 1 : -3);
    else if (ans.budget === 'b2') sc += (p > 45000 && p <= 92000) ? 4 : -1;
    else if (ans.budget === 'b3') sc += p >= 88000 ? 4 : -1;
    // вес ездока ↔ макс. нагрузка
    var load = s.load || 100;
    if (ans.weight === 'h') sc += load >= 130 ? 3 : (load >= 120 ? 1 : -3);          // 100+ кг
    else if (ans.weight === 'm') sc += load >= 120 ? 2 : (load >= 110 ? 0 : -1);     // 80–100 кг
    else if (ans.weight === 'kid') sc += -(s.power || 600) / 700 - (s.weight || 15) / 30; // лёгкие/небыстрые
    // сценарий
