...
const SCENARIOS = [
  ...
  ['📦', 'Курьерам', c => c.specs.cargo === true || (c.specs.seat === true && (c.specs.load || 0) >= 120 && (c.specs.range || 0) >= 45)],
...
function utpOf(c) {
  ...
  // «Для курьера» — только курируемые модели с багажником/грузовой платформой (specs.cargo из build_catalog).
  // Остальные велосипеды: фэты — прогулочные, обычные — городские (Андрей 05.07: V-серия без багажника ≠ курьер).
  if (s.cargo) return ['📦', 'Для курьера', 'cur'];
  if (c.cat === 'трицикл') return ['🧺', 'Устойчивый · для города и дачи', 'city'];
  if (c.cat === 'велосипед') {
    if (/фэт|fat/i.test(c.name) || (s.wheel || 0) >= 20) return ['🌲', 'Для прогулок', 'city'];
...
function openModel(id) {
  ...
  const kitRows = c.warranty
    ? '<tr><td>Гарантия</td><td>12 мес (АКБ и контроллер — 6 мес)</td></tr>' +
